...
int main() {
  ...
  while(true){
    hid_task();
    tud_task();
    led_blinking_task(blink_interval_ms);
  }
  return 0;
}
...
void hid_task(void) {
    ...
    start_ms = to_ms_since_boot(get_absolute_time()) + interval_ms;
    ...
    /*------------- Joystick -------------*/
    if (tud_hid_ready()) {
      tud_hid_gamepad_report(0x00, get_twist(), 0, 0, 0, 0, 0, 0, get_buttons());
    }
}
